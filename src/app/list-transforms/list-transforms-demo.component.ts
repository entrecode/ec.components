...
@Component({
  template: `
    ...
    <ul>
      <li *ngFor="let word of wordList?.selection?.items">{{ word.resolve('word') }}</li>
    </ul>
  `,
})
        ...
        }),
      {
        ...
        fields: {
          ...
          flow: {
            label: 'Fluss',
            resolve: (body, item) =>
              (item.sort('vocales') * item.sort('fast_consonants')) /
              Math.max(1, body.word.length * item.sort('fast_consonant_diversity') * item.sort('vocale_diversity')),
            display: percent,
            sortable: true,
          },
        },
