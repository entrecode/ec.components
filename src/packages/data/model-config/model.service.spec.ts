...
describe('ModelConfig', () => {
  ...
  it('should be able to set and get configurations', () => {
    // ModelService.set('duck', { fields: { name: 'Custom Name' } });
    // expect(ModelService.get('duck').fields.name).toBe('Custom Name');
  });

  it('should generate a field config from scratch', () => {
    /*const s = sinon.stub(Datamanager, 'schema');
    s.returns(muffinSchema);
    return ModelService.generateFieldConfig('muffin').then((config) => {
