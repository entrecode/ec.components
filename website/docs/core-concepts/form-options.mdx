...
| formComponent | custom input component used in form only        | Component               |
...
If you want custom behaviour that is not handled by the default inputViews, you can use a custom component as input:
...
In the above custom-input.component template, we use _ec-opening-hours_ with a formControl input.
